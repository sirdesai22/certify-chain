...
export default function InstitutionCertificateDetailPage() {
  ...
  const {
    ...
    numericCertificateId ? { certificateId: numericCertificateId } : { limit: 0 }
  );
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-6">
        <div className="flex items-center justify-between">
          <Button asChild variant="ghost" className="w-fit">
            <Link href="/institution/certificates">Back to Issued Certificates</Link>
          </Button>
        ...
        {certificate && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Issued to {certificate.student?.name ?? certificate.student_wallet ?? "Unknown Student"}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 text-sm text-muted-foreground">
              <div className="grid gap-2 md:grid-cols-2">
                ...
                <p>Institution Wallet: {certificate.institution_wallet}</p>
              </div>
              ...
              <div className="space-y-2">
                <h2 className="text-base font-semibold text-foreground">Recent Verification Activity</h2>
                {logsLoading && <p className="text-xs text-muted-foreground">Loading verification logs...</p>}
                {!logsLoading && logs.length === 0 && (
                  <p className="text-xs text-muted-foreground">No verification attempts yet.</p>
                )}
                <div className="space-y-2">
